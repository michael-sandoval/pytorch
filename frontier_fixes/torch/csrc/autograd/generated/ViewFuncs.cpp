...
std::vector<at::Tensor> _NestedViewFromJaggedViewFunc::get_tensors() const {
  ::std::vector<at::Tensor> tensors;
  tensors.reserve(1 + 1 + (lengths.has_value() ? 1 : 0));
  tensors.push_back(offsets);
...
size_t _NestedViewFromJaggedViewFunc::num_tensors() const {
  return static_cast<size_t>(1 + 1 + (lengths.has_value() ? 1 : 0));
}
...
at::Tensor _NestedViewFromJaggedViewFunc::operator()(const at::Tensor& input_base) const {
  return at::_ops::_nested_view_from_jagged::call(input_base, offsets, dummy, lengths, ragged_idx);
}

std::unique_ptr<ViewFunc> _NestedViewFromJaggedViewFunc::clone_and_set(
    c10::optional<std::vector<c10::SymInt>> symints,
    c10::optional<std::vector<at::Tensor>> tensors) const {
  auto output = std::make_unique<_NestedViewFromJaggedViewFunc>(offsets, dummy, lengths, ragged_idx);
  if (symints.has_value()) {
    output->set_symints(std::move(*(symints)));
  }
  if (tensors.has_value()) {
